        ...
        (if-not (seq? ast)
          ...
          (let [[kword a1 a2] ast]
            (condp = kword
              ...
              'try* (let [[catch B C] a2]
                      (if (= catch 'catch*)
                        (try (EVAL a1 env)
                             ; Mal exception
                             (catch clojure.lang.ExceptionInfo e
                               (let [let-env (env/env-new env)]
                                 (env/env-set! B (:value (ex-data e)) let-env)
                                 (EVAL C let-env)))
                             ; 'Native' exception
                             (catch Throwable e
                               (let [let-env (env/env-new env)]
                                 (env/env-set! B (or (.getMessage e) (.toString e)) let-env)
                                 (EVAL C let-env))))
                        (do (println "no catch") (EVAL a1 env))))
...
(defn repl-loop []
  ...
  (when-let [l (readline/readline "")]
    (try
      (println (rep l))
      ; Mal exception
      (catch clojure.lang.ExceptionInfo e (println "Error:" (ex-data e)))
      ; 'Native' exception
